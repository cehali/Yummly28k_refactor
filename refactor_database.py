...
for n in range(1, 10):
    ...
    rcp.get('meta0000' + str(n))['course_diet_temp'] = crs
...
for n in range(10, 100):
    ...
    rcp.get('meta000' + str(n))['course_diet_temp'] = crs
    list_recipes.append(rcp)
...
for n in range(100, 1000):
    ...
    rcp.get('meta00' + str(n))['course_diet_temp'] = crs
    list_recipes.append(rcp)
...
for n in range(1000, 10000):
    ...
    rcp.get('meta0' + str(n))['course_diet_temp'] = crs
    list_recipes.append(rcp)
...
for n in range(10000, 27639):
    ...
    rcp.get('meta' + str(n))['course_diet_temp'] = crs
    list_recipes.append(rcp)
...
for rec, rec_ingr in itertools.izip(list_recipes, recipes_ingr_mapped_name):
    ...
    courseTemp = rec.get(id)['course_diet_temp']
    idxCD = []
    for ct in courseTemp:
        idxCD.append(str(ct) + '_everything')

    dictTemp = {idxCD[0]: idxCD[0]}
    rec.get(id)['course_diet'] = dictTemp

    for i in range(1, len(idxCD)):
        dictTemp = {i: i}
        rec.get(id).get('course_diet').update(dictTemp)

    del rec.get(id)['course_diet_temp']
    ...
    if not meat_inter:
        idxCD = []
        for ct in courseTemp:
            idxCD.append(str(ct) + '_withoutMeat')
        for i in idxCD:
            dictTemp = {i: i}
            rec.get(id).get('course_diet').update(dictTemp)
        ...
        if not fish_inter:
            idxCD = []
            for ct in courseTemp:
                idxCD.append(str(ct) + '_vegetarian')
            for i in idxCD:
                dictTemp = {i: i}
                rec.get(id).get('course_diet').update(dictTemp)
    ...
    if not fish_inter:
        idxCD = []
        for ct in courseTemp:
            idxCD.append(str(ct) + '_withoutFish')
        for i in idxCD:
            dictTemp = {i: i}
            rec.get(id)['course_diet'].update(dictTemp)
    ...
    if not dairy_inter:
        idxCD = []
        for ct in courseTemp:
            idxCD.append(str(ct) + '_withoutDiary')
        for i in idxCD:
            dictTemp = {i: i}
            rec.get(id)['course_diet'].update(dictTemp)
...
with open('recipes_withDishType_withoutParent_specialIndex_3.json', 'w') as outfile:
